...
public class PointInRegionMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
	...
	private int queryQuadTree(Point pt) {
		...
		while (elmHandle >= 0) {
			...
			if (GeometryEngine.contains(
					featureClass.features[featureIndex].geometry, pt,
					spatialReference)) {
				return featureIndex;
			}
	...
	public void map(LongWritable key, Text val, Context context)
		...
		// Note: We know the data coming in is clean, but in practice it's best
		// not to
		...
		// Each map only processes one earthquake record at a time, so we start
		// out with our count
		...
		if (featureIndex >= 0) {
			String name = (String) featureClass.features[featureIndex].attributes
					.get(labelAttribute);
